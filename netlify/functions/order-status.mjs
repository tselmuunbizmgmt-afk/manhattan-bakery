...
export default async (req) => {
  ...
  const order = await ORDERS_STORE().get(id, { type: 'json', consistency: 'strong' });
  ...
  return json({
    id: order.id, status: order.status, paymentStatus: order.paymentStatus || (order.status === 'received' ? 'paid' : 'unknown'), updatedAt: order.updatedAt, createdAt: order.createdAt,
